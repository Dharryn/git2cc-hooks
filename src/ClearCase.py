...
class ClearCase:
  ...
  def checkout (self, ccpath, comment, addVersion=False):
    ...
    try:
      ...
      raise CCError(self._("co") + " " + ccpath + self._("command_failed") + 
                    str(sys.exc_info()))
  ...
  def checkin (self, ccpath):
    ...
    try:
      ...
      p = subprocess.Popen([self._config.get_cleartool_path(), "ci", "-nc",
                            ccpath],
                           stdout=subprocess.PIPE,
                           stderr=subprocess.PIPE)
      ...
      raise CCError(self._("ci") + " " + ccpath + self._("command_failed") + 
                    str(sys.exc_info()))
  ...
  def create_dir (self, ccpath):
    ...
    if not os.path.isdir(ccpath):
      ...
      try:
        ...
        p = subprocess.Popen([self._config.get_cleartool_path(), "mkdir", "-c",
                             self._("new_CC_folder"), ccpath],
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)
        out, err = p.communicate()
        ...
        raise CCError(ccpath + self._("creation_failed") + str(sys.exc_info()))
  ...
  def create_path (self, ccpath):
    ...
    # Because ccpath starts with os.sep the first element in dirs will be the ""
    # and must be deleted
    del dirs[0]
  ...
  def create_file (self, ccpath):
    ...
    try:
      ...
      with open(ccpath) as f:
        p = subprocess.Popen([self._config.get_cleartool_path(), "mkelem",
                              "-nc", "-nco", ccpath],
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)
        out, err = p.communicate()
      ...
      if p.returncode == 0:
        """ 
        Previous operation puts an empty new file in the main ClearCase branch
        leaving the actual file as ccpath.keep
        We must add the content to our branch performing co then ci of the
        keep file.
          
        """
        self.checkout(ccpath + "@@/main/LATEST", self._("new_file"), True)
    ...
    for i in os.listdir(self._config.get_view()):
      ...
      if os.path.isdir(current_path) and self.is_versioned(current_path):
        ...
        try:
          ...
          # Get every checked out children (file/folder) of current path
          p = subprocess.Popen([self._config.get_cleartool_path(),
                                "lsco", "-s", "-r", "-cvi", current_path],
                               stdout=subprocess.PIPE,
                               stderr=subprocess.PIPE)
          out, err = p.communicate()
          ...
          raise CCError("ct lsco -s -r -cvi " + self._config.get_view() + 
                        self._("command_failed") + str(sys.exc_info()))

        if p.returncode == 0:

          if not (out == None or out == ""):
  ...
  def checkin_all(self):
    ...
    for co in colist:
      ...
      try:
        ...
        print self._("WARNING") + " " + co + self._("impossible_ci")
        print e
        continue
  ...
  def uncheckout_all(self):
    ...
    for co in colist:
      ...
      try:
        ...
        print self._("WARNING") + " " + co + self._("impossible_unco")
        print e
        continue
      ...
      if os.path.isdir(co) and not os.listdir(co) and not self.is_versioned(co):
  ...
  def remove_name(self, ccpath):
    ...
    if p.returncode != 0:
      ...
      raise CCError("ct rmname " + ccpath + self._("command_failed") + str(err))
