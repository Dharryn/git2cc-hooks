...
class GIT:
  ...
  def get_commit_files (self, old_revision, new_revision):
    ...
    try:
      ...
      raise GITError("git diff" + self._("command_failed") + str(sys.exc_info()))
    ...
    if p.returncode == 0:
      ...
      for revision in revisions.splitlines():
        ...
        try:
          ...
          p1 = subprocess.Popen(["git", "cat-file", "commit", revision],
                                stdout=subprocess.PIPE)
        ...
        if p.returncode == 0:
          ...
          raise GITError("git cat-file" + self._("command_failed") + str(err))
      ...
      raise GITError("git rev-list" + self._("command_failed") + str(err))
  ...
  def list_deletions(self, old_revision, new_revision):
    ...
    try:
      ...
      p = subprocess.Popen(["git", "diff-tree", "-t", old_revision,
                            new_revision, "--diff-filter=D", "--name-only"],
                           stdout=subprocess.PIPE,
                           stderr=subprocess.PIPE)
